...
@testset "SignalPressureMapping.jl" begin
    ...
    @test Detectability.signal_to_pressure_level(mapping, Array{Float64}([1.0, 10.0])) == Array{Float64}([100.0, 120.0])
    @test Detectability.pressure_to_signal_level(mapping, Array{Float64}([100.0, 120.0])) == Array{Float64}([1.0, 10.0])
end

@testset "ThresholdInQuiet.jl" begin
    sampling_rate::Float64 = 48000
    n_samples::Int64 = 10
    frequencies::Array{Float64} = DSP.Util.rfftfreq(n_samples, sampling_rate)
    mapping::Detectability.SignalPressureMapping = Detectability.SignalPressureMapping(1.0, 100.0) 
    @test Detectability.threshold_in_quiet_signal_level(mapping, frequencies)[2:end-1] ≈ Array{Float64}([9.86684635e-06, 2.84757976e-05, 1.48407282e-03,6.48469551e+01])
end
...
@testset "OuterMiddleEarFilter.jl" begin
    ...
    frequencies::Array{Float64} = DSP.Util.rfftfreq(n_samples, sampling_rate)
    mapping::Detectability.SignalPressureMapping = Detectability.SignalPressureMapping(1.0, 100.0) 
    @test Detectability.outer_middle_ear_filter(mapping, frequencies) ≈ Array{Float64}([0.00000000e+00, 1.01349506e+05, 3.51175414e+04, 6.73821383e+02, 1.54209245e-02, 2.49391323e-12])
end
